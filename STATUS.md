...
Current repository state: `PRIVATE_PR_PDF_INTEGRATION_CI_PENDING`.
...
| Remote/publication | `PRIVATE_DRAFT_PR_AUDITED` | Private draft PR #1 exists. The repaired exact-source head passed all four checks and its PDF passed independent audit. The later PDF-integration head must repeat all four checks. No public repository, merge, tag, immutable release, or DOI is authorized yet. |

## Required next gates

1. Require all four checks to remain green on the exact accepted-PDF
   integration head.
